...
fun Application.module(testing: Boolean = false) {
    val account = Account()
    val marketClient = MarketClient("http://0.0.0.0:8080", account)
    ...
    suspend fun getUserStocks(user: User): List<Stock> {
        val userStocks = user.stocks.map {
            val stocksInfo = marketClient.getStocksInfo(it.key)
    ...
    routing {
        ...
        post("/buy") {
            try {
                ...
                marketClient.buy(userId.toInt(), companyName, quantity.toInt())
                call.respond(HttpStatusCode.OK)
            } catch (e: Exception) {
                call.respond(HttpStatusCode.BadRequest, e.message ?: "")
            }
        ...
        post("/sell") {
            try {
                ...
                marketClient.sell(userId.toInt(), companyName, quantity.toInt())
                call.respond(HttpStatusCode.OK)
            } catch (e: Exception) {
                call.respond(HttpStatusCode.BadRequest, e.message ?: "")
            }
